...
$(function () {
  ...
  if ($brand.isOnScreen(0.5)) {
    $brand.addClass("active");
  }

  $(window).on("scroll", function () {
    if ($brand.hasClass("active")) return;
...
$(function () {
  let bodyHeight = $(".h-welcome__body").height();
  let descHeight = $(".h-welcome__desc").height();

  if (descHeight > bodyHeight) {
    $(".h-welcome__body").addClass("is-collapse");
  }
});
...
// project preview sync slider
$(function () {
  if (!$(".project-preview-slider, .project-thumb-slider").length) {
    return;
  }
  ...
  addSwiper(".project-preview-slider", {
    effect: "fade",
    allowTouchMove: false,
    thumbs: {
      swiper: thumbSlider
