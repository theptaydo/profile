export default {
  header: {
    home: "首页",
    about: "关于我们",
    service: "服务",
    profile: "档案",
    team: "团队",
    products: {
      main: "产品",
    },
    contact: "联系我们",
    switchLanguage: "越南",
    contact: "联系我们",
    switchLanguage: {
      lang3: {
        id: "vi",
        name: "Việt Nam",
        src: "/img/langs/Flag_of_Vietnam.svg.png"
      },
      lang1: {
        id: "en",
        name: "English",
        src: "/img/langs/Flag_of_the_United_Kingdom.png"
      },
      lang2: {
        id: "km",
        name: "កម្ពុជា",
        src: "/img/langs/Flag_of_Cambodia.svg.png"
      },  //khmer
      lang: {
        id: "zh-cn",
        name: "中国",
        src: "/img/langs/china.png"
      },  //china
      lang4: {
        id: "ja",
        name: "日本",
        src: "/img/langs/Flag_of_Japan_(bordered).svg.png"
      },  //japan
    },
  },
  footer: {
    section: [
      {
        brand: "Tay Do Steel",
        address: "123 Industrial Street, Binh Thuy District, Can Tho City",
        phone: {
          title: "Phone: ",
          value: "+84 292 123 4567",
        },
        email: {
          title: "Email: ",
          value: "info@theptaydo.com"
        }
      },
      {
        brand: "Scrap Processing Workshop",
        address: "27 Le Hong Phong, Tra An Ward, Binh Thuy District, Can Tho City",
        phone: {
          title: "Phone: ",
          value: "+84.796 65 65 65",
        },
        email: {
          title: "Email: ",
          value: "info@theptaydo.com"
        }
      },
      {
        brand: "Representative office in Can Tho",
        address: "No. 44, CMT8, Cai Khe Ward, Ninh Kieu District, Can Tho City",
        phone: {
          title: "Phone: ",
          value: "+84.796 65 65 65",
        },
        email: {
          title: "Email: ",
          value: "info@theptaydo.com"
        }
      },
      {
        brand: "Representative office in Ho Chi Minh city",
        address: "...",
        phone: {
          title: "Phone: ",
          value: "+84.796 65 65 65",
        },
        email: {
          title: "Email: ",
          value: "info@theptaydo.com"
        }
      },
    ],
    cert: {
      title: "营业执照编号: 1800156657",
      dateOfPlace: "发证日期: 2008年6月27日 - 发证机构: 芹苴市计划与投资局",
      grant: "总经理: 黄忠光"
    },
    infor: {
      title: "关注我们",
      description: "通过社交媒体与我们联系，获取最新的钢铁市场信息。",
    },
    coppyright: {
      brand: "Tay Do Steel",
      title: "版权",
      subTitle: "已注册版权",
      developer: "设计者："
    }
  },

  HomePage: {
    home: "首页",
    contact: "联系我们",
    switchLanguage: "越南",
    hero: {
      welcome: "欢迎来到 ",
      brand: "Tay Do Steel", //西都钢铁
      description: "湄公河三角洲地区领先的建筑钢铁生产和销售企业。",
      buy: "立即购买",
      more: "了解更多",
    },
    featuredServices: {
      one: {
        title: "建筑钢材供应",
        description: "专业提供各种建筑钢材，如钢筋、型钢和钢卷，适用于大中小型工程。",
      },
      two: {
        title: "定制钢材加工",
        description: "提供根据客户需求定制的钢材加工服务，满足精确的技术标准。",
      },
      three: {
        title: "钢材解决方案咨询",
        description: "我们提供深入的咨询，针对每种工程提供合适的钢材类型。",
      },
      four: {
        title: "钢材运输和安装",
        description: "快速的运输和现场钢材安装服务，拥有专业团队。",
      },
    },
    about: {
      badge: "介绍",
      title: "关于 ",
      subTitle: "我们",
      hightlight: "钢铁行业的先锋",
      description: "拥有超过20年的经验，西多钢铁专业提供高质量的钢铁产品，用于建筑和工业。",
      attributes: {
        one: {
          title: "顶级质量",
          description: "钢铁产品按照国际标准进行检测，确保耐用性和安全性。",
        },
        two: {
          title: "具有竞争力的价格",
          description: "我们承诺以市场上最具竞争力的价格提供产品。",
        }
      },
      message: "西多钢铁始终承诺高质量，确保客户满意，并建造可持续、安全的工程。",
    },
    team: {
      badge: "团队",
      title: "我们的主要成员 ",
      subTitle: "我们",
      attributes: [
        {
          name: "Dao Duy Tuan Hung",
          role: "副总经理 - 质量管理",
          images: "/img/team/hungddt.png",
          titles: "先生 "
        },
        {
          name: "Huynh Trung Quang",
          role: "总经理兼董事会主席",
          images: "/img/team/quanght.png",
          titles: "先生 "
        },
        {
          name: "Vo Van Khai",
          role: "副总经理 - 销售",
          images: "/img/team/khaivv.png",
          titles: "先生 "
        },
        {
          name: "Anh Da Den",
          role: "Uvuvwevwevwe Onyetenyevwe Ugwemubwem Osas",
          images: "https://th.bing.com/th/id/OIP.ueglsKxAkWD-4Bb_nc46IgHaHa?rs=1&pid=ImgDetMain",
          titles: "先生 "
        },
      ]
    },
    portfolio: {
      badge: "项目",
      title: "探索",
      subTitle: "我们的项目",
      attributes: [
        {
          name: "芹苴大桥",
          date: "....",
          images: "/img/portfolio/cau-can-tho-su-dung-thep-tay-do.png"
        },
        {
          name: "芹苴大桥",
          date: "....",
          images: "/img/portfolio/da-khoa-tpct-su-dung-thep-tay-do.png"
        },
        {
          name: "芹苴大桥",
          date: "....",
          images: "/img/portfolio/da-khoa-trung-uong-ct-su-dung-thep-tay-do.png"
        },
        {
          name: "芹苴大桥",
          date: "....",
          images: "/img/portfolio/dai-hoc-can-tho-su-dung-thep-tay-do.png"
        },
        {
          name: "芹苴大桥",
          date: "....",
          images: "/img/portfolio/cau-can-tho-su-dung-thep-tay-do.png"
        },
      ]
    },
    faq: {
      badge: "常见问题",
      title: "关于的常见问题",
      subTitle: "西都钢铁",
      attributes: [
        {
          title: "西都钢铁的产品是否符合国际质量标准？",
          value: "我们的所有钢铁产品均符合 ASTM、JIS 和 EN 等国际质量标准，确保所有建筑项目的耐用性和安全性。"
        },
        {
          title: "西都钢铁是否提供定制钢材加工服务？",
          value: "我们根据客户的技术要求提供定制钢材加工服务，包括切割、弯曲和镀锌，满足每个项目的所有标准和要求。"
        },
        {
          title: "西都钢铁的交货时间是多久？",
          value: "标准交货时间为 3-5 个工作日，具体取决于地点和订单数量。我们还根据客户要求提供快速运输服务。"
        },
        {
          title: "西都钢铁是否支持大型项目的咨询？",
          value: "我们拥有高技能的技术团队，随时为全国各类建设项目提供产品建议和合适的解决方案。"
        },
        {
          title: "西都钢铁是否提供钢材维护服务？",
          value: "我们提供定期维护和检查服务，帮助客户确保钢材产品使用后的质量和寿命。"
        },
        {
          title: "西都钢铁是否为国际项目提供钢材？",
          value: "我们不仅为国内市场提供钢材，还出口到其他国家，符合最严格的国际标准。"
        },
      ]
    }





  }
}
